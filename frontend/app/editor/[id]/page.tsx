...
export default function EditorWithIdPage() {
  ...
  const LeftPanelContent = (
    <div className="bg-[#282828] rounded-xl border border-[#383838] flex flex-col overflow-hidden h-full">
  ...
  const ConsoleContent = (
    <div className="bg-[#282828] flex flex-col h-full rounded-xl overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${
              activeConsoleTab === "input" ? "bg-[#333] text-white" : "text-zinc-400 hover:text-white"
            }`}
          >
            Custom Input
          </button>
          ...
          {activeConsoleTab === "input" ? (
            <div className="h-full flex flex-col flex-1">
              ...
              <Textarea 
                ...
                className="flex-1 min-h-[100px] bg-[#1e1e1e] border-[#383838] text-zinc-300 font-mono text-sm resize-none focus:border-blue-500 custom-scrollbar"
              />
            </div>
      ...
      <div className="min-h-screen bg-[#1a1a1a] text-[#eff1f6] flex flex-col h-screen overflow-hidden font-sans">
        ...
        <header className="bg-[#282828] border-b border-[#383838] px-4 h-12 flex items-center justify-between shrink-0 select-none z-50">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-sm text-white tracking-wide truncate max-w-[150px] sm:max-w-xs">{localTitle || "Practice Sandbox"}</span>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Select value={localLanguage} onValueChange={handleLanguageChange}>
              <SelectTrigger className="w-[90px] sm:w-[120px] bg-[#333] border-[#444] text-white rounded-md text-xs h-7 focus:ring-0">
                <SelectValue />
              </SelectTrigger>
            ...
            >
              {isRunning ? (
                ...
                <>
                  <Play className="w-3.5 h-3.5 sm:mr-1.5 fill-white" />
        ...
        <div className="md:hidden flex bg-[#282828] border-b border-[#383838] shrink-0">
          <button
        ...
        <div className="hidden md:grid flex-1 grid-cols-12 overflow-hidden bg-[#1a1a1a] p-1.5 gap-1.5">
          <div className="col-span-5 h-full overflow-hidden">
            {LeftPanelContent}
          </div>
          <div className="col-span-7 flex flex-col h-full overflow-hidden gap-1.5">
            <div className="flex-1 overflow-hidden min-h-[350px] border border-[#383838] rounded-xl flex flex-col">
              <CodeEditor
                value={localCode}
            ...
            <div className={`border border-[#383838] rounded-xl flex flex-col transition-all ${
              isConsoleExpanded ? "h-64 lg:h-72" : "h-10"
            }`}>
              {ConsoleContent}
            </div>
          </div>
        ...
        <div className="md:hidden flex-1 overflow-hidden flex flex-col p-1.5 gap-1.5">
          <div className={`flex-1 overflow-hidden ${mobileTab === "problem" ? "flex" : "hidden"}`}>
            {LeftPanelContent}
          </div>
          
          <div className={`flex-1 flex-col h-full overflow-hidden ${mobileTab === "code" ? "flex" : "hidden"}`}>
            <div className="flex-1 overflow-hidden border border-[#383838] rounded-xl flex flex-col">
              <CodeEditor value={localCode} onChange={handleCodeChange} language={localLanguage} />
            </div>
          </div>

          <div className={`flex-1 flex-col h-full border border-[#383838] rounded-xl overflow-hidden ${mobileTab === "output" ? "flex" : "hidden"}`}>
            {ConsoleContent}
          </div>
        </div>
      </div>
